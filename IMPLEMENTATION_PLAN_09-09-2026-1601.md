...
**Phase 9.5 status:** COMPLETE (2026-09-07)
...
**Implementation (2026-09-07):** Added exactly one bearer-protected creation operation, `POST /api/v1/pre-triage/episodes/{episodeId}/check-ins`. `RecordSymptomCheckIn` first reuses Phase 9.4 completed, persisted patient-owned episode resolution and Phase 3 authorization, then begins a PostgreSQL transaction serialized by an episode/idempotency-key advisory lock, share-locks the episode, and reauthorizes active managed access under the existing relationship lock. The pathway comes only from the episode's frozen questionnaire. The request accepts exactly non-empty UUID `packageVersionId`, UUID `idempotencyKey`, and an ordered `answers` array of `{ questionCode, value }`; extension fields and query selectors are rejected. Exact-version lookup uses `ISymptomDiaryContentProvider.GetExactPackageAsync` and never substitutes the newest active package. A historically presented version remains submittable only while its immutable provenance is internally valid, reviewed, approved, and activated and its pathway exactly matches the episode. OTHER_SYMPTOMS, unknown, wrong-pathway, inactive, unapproved, corrupt, or inconsistent packages fail closed without fallback. Structural validation enforces known same-package question codes, no duplicates, package-defined required/optional semantics, string/single-choice/string-array shapes, exact option values, unique multi-choice values, and persistence limits; it performs no medical validation. Accepted values are stored as JSON against exact same-package question/package foreign keys. A canonical SHA-256 over package identity and source-ordered logical answers backs the existing `(episode_id, idempotency_key)` uniqueness: identical retries return the original entry with `200`, materially different reuse returns safe `409`, and concurrent identical requests create one row. First creation atomically appends the check-in and all supplied answers with server-controlled UTC time and returns `201`; omitted optional answers create no rows. The neutral response contains check-in/episode/package/pathway/time, immutable package/hash/provenance and question-set/information identities, ordered accepted question definitions/options/values, and separate static warning information. No update/delete/history route, comparison, interpretation, warning matching, assessment, recommendation, escalation, reminder, notification/outbox, AI, FHIR, Clinical History projection, or sensitive-answer logging was added. No Phase 9.5 migration was required.

**Verification (2026-09-07):** Focused Phase 9.5 coverage passed 29/29 unit tests, including the 3/3 dedicated architecture/safety filter, and 15/15 real-PostgreSQL API tests. Relevant Phase 9.1-9.4 regressions passed 69/69 unit and 55/55 PostgreSQL tests; relevant Phase 4 definition and authorization/Pre-Triage regressions passed 22/22 unit and 69/69 PostgreSQL tests. The complete unit suite passed 1,206/1,206 and the complete PostgreSQL integration suite passed 727/727, with zero failures and zero skipped tests. Dedicated migration regressions passed 23/23 and OpenAPI/CORS regressions passed 6/6. OpenAPI changed only from 52 to 53 paths by adding the intended POST on the new `/check-ins` path; no history GET exists. EF reports no pending model changes; locked restore, the final Debug build with zero warnings/errors, `dotnet format --verify-no-changes`, and `git diff --check` passed. No migration or model snapshot changed, and `docs/symptoms.md` retained SHA-256 `DD851AA5E1EE2DAAC98926314B287E713D2E6C13C6F646AD4F1FC0E260907A62`.

**Remaining blockers (2026-09-07):** None for Phase 9.5. Phase 9 remains in progress and Phase 9.6 remains not started.

## Phase 9.6 — Longitudinal Diary History Retrieval

**Phase 9.6 status:** COMPLETE (2026-09-07)
...
**Implementation (2026-09-07):** Added only bearer-protected `GET /api/v1/pre-triage/episodes/{episodeId}/check-ins` on the existing Phase 9.5 path. `ListSymptomCheckIns` first resolves the eligible completed patient-owned episode through the Phase 9.4 boundary and reevaluates current Phase 3 Primary or active Managed authority; missing, ineligible, unrelated, reverse, revoked, and otherwise inaccessible episodes retain the same concealed `404`, including when paging selectors are invalid. The endpoint accepts exactly one optional `cursor` and `pageSize`, rejects a body and unsupported/repeated selectors, defaults to 20, and caps at 100. Its deterministic HMAC-protected opaque cursor is derived with domain separation from the configured application signing key and binds cursor version, episode, normalized page size, and the exact `(createdAt, id)` boundary; malformed, tampered, stale, cross-episode, and page-size-incompatible cursors return safe `422`. PostgreSQL performs an oldest-first no-tracking keyset query with `LIMIT pageSize + 1` over the existing `(episode_id, created_at, id)` index, then one bounded answer query. Distinct package IDs from the displayed page are resolved by a bounded exact-version graph batch, avoiding per-entry package lookup. Every item is reconstructed only from its stored immutable `packageVersionId`; exact pathway, reviewed/approved/activated provenance, canonical hash, questions, schemas, ordered options, question-set/information identities, general information, ordered warning signs, persisted answer/question/order linkage, and submitted JSON are verified or preserved. Missing, corrupt, cross-pathway, unapproved, or internally inconsistent frozen content fails closed as sanitized `422` and emits only episode ID plus package-count audit metadata. The response is exactly `{ items, nextCursor }`, with each item retaining the Phase 9.5 neutral response shape and omitting submitter account identity, source/import paths, idempotency/request hashes, canonical JSON, clinical derivations, comparisons, warning matches, recommendations, reminders, AI/provider data, FHIR, and Clinical History output. GET performs no mutation. No migration, domain model, model snapshot, or `docs/symptoms.md` change was required.

**Verification (2026-09-07):** Focused Phase 9.6 coverage passed 11/11 application unit tests, 3/3 architecture/safety tests, and 13/13 authenticated real-PostgreSQL/API tests. Phase 9.2 exact-version regressions passed 22/22 unit and 11/11 PostgreSQL tests; the combined Phase 9.4-9.6 regressions passed 57/57 unit and 39/39 PostgreSQL tests; targeted patient-authorization/Phase 4 regressions passed 47/47 unit and 24/24 PostgreSQL tests. The complete unit suite passed 1,220/1,220 and the complete PostgreSQL integration suite passed 740/740, with zero failures and zero skipped tests. Dedicated migration regressions passed 23/23 and OpenAPI/CORS regressions passed 6/6. OpenAPI remains exactly 53 paths before and after Phase 9.6 and adds only GET on the existing `/check-ins` path, with Bearer security, episode/cursor/integer-page-size parameters, the exact page schema, and `200`/`401`/`404`/`422`/`500`. Locked restore, the final Debug build with zero warnings/errors, EF pending-model-change check, `dotnet format --verify-no-changes` over 755 files, and `git diff --check` passed. No migration or model snapshot changed, and `docs/symptoms.md` retained SHA-256 `DD851AA5E1EE2DAAC98926314B287E713D2E6C13C6F646AD4F1FC0E260907A62`.

**Remaining blockers (2026-09-07):** None for Phase 9.6. Phase 9 remains in progress and Phase 9.7 remains not started.
...
**Phase 11.4 is complete. Phase 11.5 has not started.**
...
**COMPLETE (2026-09-09).** Phase 11.3 and the listed clinical read dependencies were verified before implementation.
...
**Implementation (2026-09-09):** Added exactly ShareAccess-secured `GET /api/v1/shared-access/profile` with no query/body scope or record override. `BuildSharedProfile` resolves the stable grant ID, immutable token scope, and UUID `jti` from the already validated Phase 11.3 principal, reloads the current ShareGrant and exact stored items on every request, rechecks future/revoked/expired state at the exact boundary, requires token/grant scope equality, and delegates to the reusable fail-closed `IShareScopeEvaluator`. `FullProfile` accepts no items; `PreTriage` requires only exact `pre_triage_episode` items; `SpecificRecords` requires exact supported stored items; `Case`, `Visit`, missing/corrupt/foreign/unsupported records, duplicate/inconsistent items, and legacy metadata-only PreTriage grants cannot fall back to a broader scope. The provider-neutral `ICanonicalSharedHealthSnapshotBuilder` is the single reusable Beeexy JSON/PDF-ready allow-list boundary. It composes existing PatientProfile, Clinical History, completed patient-owned/claimed Pre-Triage, immutable Symptom Diary exact-version, and displayable approved Second Opinion read semantics; returns deterministic oldest-first records with truthful nullability and minimum immutable version/provenance; presents approved diary warning/informational content separately from exact submitted values; and never serializes EF/domain rows. Explicit DTOs exclude account/session/security data, capabilities/tokens/hashes used for authority, appointments/directory/notifications/preferences, temporary or unclaimed Pre-Triage, AI conversations/messages/prompts/provider/model/raw/rejected/safety internals, source/import paths, storage identities, audit internals, logs, and telemetry. Second Opinion projection selects only the latest approved displayable immutable snapshot per analysis for FullProfile and the exact approved snapshot for an explicit item, then parses only patient-visible result fields without provider execution or reinterpretation. The endpoint sets `Cache-Control: no-store`; safe grant/source failures remain one concealed shared-access `401`; unexpected failures remain sanitized by the existing Problem Details boundary. The existing ShareAccess scheme cannot authenticate representative patient, diary, AI, appointment, or share mutations. Each successful logical access records only a `ShareAccessed/Succeeded` event with no resource/payload/credential metadata; its deterministic SHA-256-derived UUID from `(grantId, token jti)` makes repeated/concurrent execution of the same issued token idempotent while a newly exchanged token remains a distinct access identity. No activity listing, revocation route, export bytes/storage, PDF, QR, FHIR, provider call, content copy, schema change, or migration was added.
